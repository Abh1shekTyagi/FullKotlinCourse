...
fun main(){
    ...
    println()
    print(map.mapKeys { Pair(it.key * 100 , it.value)}) // we can access the keys separately, we can also access values inside
    //{(100, abhishek)=abhishek, (200, tyagi)=tyagi}

    println()
    print(map.mapValues { Pair(it.key * 40 , it.value)}) // we can access the values separately, we can also access keys inside
